...
function getUser(username, password, callback){
    ...
    user.findByUsernameAndPassword(username, password, {}, function(user, empty){
...
function saveAccessToken(bearerToken, clientID, expires, user, callback){

  console.log('saveAccessToken() called and accessToken is: ', bearerToken,' and clientID is: ',clientID, ' and user is: ', user.id, ' and accessToken is: ', bearerToken);
  ...
  accessToken.create(token, {}, function(user, empty){
      callback(false, user);
  });
}
...
function getAccessToken(bearerToken, callback) {
    ...
    accessToken.findByAccessToken(bearerToken, {}, function(data, empty){
        const token = {
            user: {
                id: data.userID,
            },
        ...
        callback(data.userID == null ? true : false, data.userID == null ? null : token)
    });
}
